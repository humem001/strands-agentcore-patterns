#!/usr/bin/env python3
"""
Package Tool Lambda deployment package with dependencies.
"""
...
def package_tool_lambda():
    """Create deployment package for Tool Lambda."""
    print("=" * 60)
    print("PACKAGING TOOL LAMBDA")
    print("=" * 60)
    
    # Paths
    package_dir = Path("tool-lambda-package")
    src_dir = Path("src")
    ...
    # Copy tool module
    tool_src = src_dir / "tool"
    tool_dst = package_dir / "tool"
    print(f"  Copying {tool_src} -> {tool_dst}")
    ...
    zip_file = "tool-lambda-deployment.zip"
    
    if Path(zip_file).exists():
        Path(zip_file).unlink()
    
    shutil.make_archive(
        "tool-lambda-deployment",
        "zip",
        package_dir
    )
    ...
    if "tool/handler.py" in result.stdout and "shared/" in result.stdout:
    ...
    print("\n" + "=" * 60)
    print("✓ TOOL LAMBDA PACKAGE READY")
    print("=" * 60)
    print(f"\nPackage: {zip_file}")
    print(f"Size: {zip_size:.2f} MB")
    print("\nNext step: python3 scripts/upload_tool_lambda.py")
    
    return True


if __name__ == "__main__":
    success = package_tool_lambda()
