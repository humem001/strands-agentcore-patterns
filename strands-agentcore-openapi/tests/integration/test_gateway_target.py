...
Tests that the WeatherAPI Gateway Target was created correctly
and is accessible via AWS API.
"""
...
def test_lambda_functions_exist(stack_outputs):
    ...
    interceptor_lambda_arn = stack_outputs['InterceptorLambdaArn']
    ...
    # Test Interceptor Lambda
    interceptor_response = lambda_client.get_function(FunctionName=interceptor_lambda_arn)
    assert interceptor_response['Configuration']['FunctionName'] == 'dev-interceptor-lambda'
    assert interceptor_response['Configuration']['Runtime'] == 'python3.12'
