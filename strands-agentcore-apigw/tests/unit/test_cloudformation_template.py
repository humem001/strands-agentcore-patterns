...
class TestLambdaConfig:
    def test_lambda_exists(self, resources):
        ...
        assert resources["AgentLambdaFunction"]["Type"] == "AWS::Lambda::Function"
...
class TestAgentLambdaRole:
    def test_role_exists(self, resources):
        assert "AgentLambdaRole" in resources
        assert resources["AgentLambdaRole"]["Type"] == "AWS::IAM::Role"

    def test_bedrock_invoke_model(self, resources):
        actions = _collect_actions_from_policies(resources["AgentLambdaRole"])
    ...
    def test_cloudwatch_create_log_group(self, resources):
        actions = _collect_actions_from_policies(resources["AgentLambdaRole"])
        assert "logs:CreateLogGroup" in actions

    def test_cloudwatch_create_log_stream(self, resources):
        actions = _collect_actions_from_policies(resources["AgentLambdaRole"])
        assert "logs:CreateLogStream" in actions

    def test_cloudwatch_put_log_events(self, resources):
        actions = _collect_actions_from_policies(resources["AgentLambdaRole"])
